...
function Pause-For-BackupReminder {
    ...
    Write-Host "  IMPORTANT: MOVE BACKUP TO A SECURE LOCATION" -ForegroundColor Yellow
    Write-Host "=============================================================" -ForegroundColor Yellow
    Write-Host (" Backup file: {0}" -f $BackupPath) -ForegroundColor White
    Write-Host "• Treat the backup as sensitive (it may contain secrets)." -ForegroundColor White
    Write-Host "• Move or copy it to a secured/limited-access location." -ForegroundColor White
    Write-Host "-------------------------------------------------------------" -ForegroundColor Yellow
    $null = Read-Host "Type OK (or press Enter) to start the countdown"
}
